...
Preprocess::Preprocess() : feature_enabled(0), lidar_type(AVIA), blind(0.01), detection(100.0), point_filter_num(1)
{
  printf("hello\n");
  inf_bound = 10;
  N_SCANS = 6;
...
void Preprocess::set(bool feat_en, int lid_type, double bld, double dct, int pfilt_num)
{
...
void Preprocess::mid360_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  for (uint i = 0; i < plsize; ++i)
  {
    ...
    yaw_last[layer] = yaw_angle;
    time_last[layer] = added_pt.curvature;
    double dis2 = added_pt.x * added_pt.x + added_pt.y * added_pt.y + added_pt.z * added_pt.z;
    
    // printf( "dis2=%.2f\n", dis2);
    if ((blind * blind) < dis2 && dis2 < (detection * detection))
    {
...
void Preprocess::give_feature(pcl::PointCloud<PointType>& pl, vector<orgtype>& types)
{
  ...
  {
    if (types[i].range < blind || types[i].range > detection)
    {
      continue;
    }
